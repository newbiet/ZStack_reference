...
根据他们的业务逻辑，插件可以选择或不创建数据库模型。例如，开放式的vSwitch L2网络，创建一个新的资源类型，可能需要一个新的模型; 然而，一个插件，允许预留物理机内存可能并不需要创建一个单独的模型，但需要添加一段数据连接到物理机。如果IaaS的软件不提供一种方式来使插件附加数据。那就需要用户创造新的，琐碎的模型或修改现有的模型来增加其列，这会导致软件升级的数据库迁移毛茸茸的情况。

最后，在ZStack配合内置的第三方软件，使他们能够信息存储到ZStack配合的数据库，而不影响数据完整性，并使用ZStack配合来查询的API。
